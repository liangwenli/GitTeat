<template>
  <div class="cart">
    <div class="cartHeader">
      <div class="h-left fl">
        <span class="el-icon-shopping-bag-2 span1"></span>
...
export default {
  ...
  methods: {
    ...
    async add(goodindex) {
      ...
      if (this.good[goodindex].num * 1 >= this.good[goodindex].kucun * 1) {
        ...
        let { data: numdata } = await this.$axios.post(
          " http://localhost:1912/goods/updata",
    ...
    settle() {
      ...
      order.forEach(async item => {
        await this.$axios.post(
          " http://localhost:1912/goods/insertorder",
          Qs.stringify(item)
        );
  ...
  async created() {
    ...
    let { data: cartdata } = await this.$axios.get(
      "http://localhost:1912/goods/cart",
      { uid: uid }
    );
